...
const initState = {
  ...
  tester: { list: [], history: [], inputs: [], current: '', typed: '' },
};

// Shuffle function
function shuffle(payload) {
  const pool = Array.from(payload);
  const s = pool.every((word) => word.includes(' '))
    ? pool.flatMap((token) => token.split(' '))
    : pool.sort(() => Math.random() - 0.5);
  return { list: s, history: [], inputs: [], typed: '', current: s[0] };
}
...
function append(state) {
  ...
  return {
    history: [...state.history, state.typed],
    current: state.list[i],
    typed: '',
  };
}
...
export const prefReducer = createReducer(initState.pref, (builder) => {
  builder
    .addCase(actions.setType, (state, { payload }) => ({
      ...state,
      type: payload,
    }))
    .addCase(actions.setLimit, (state, { payload }) => ({
      ...state,
      limit: payload,
    }))
    .addCase(actions.setTheme, (state, { payload }) => ({
      ...state,
      theme: payload,
    }));
});
...
export const timerReducer = createReducer(initState.timer, (builder) => {
  builder
    .addCase(actions.setTimer, (state, { payload }) => ({
      ...state,
      now: payload,
    }))
    .addCase(actions.setTimerId, (state, { payload }) => ({
      ...state,
      id: payload,
    }))
...
export const testerReducer = createReducer(initState.tester, (builder) => {
  builder
    .addCase(actions.setWord, (state, { payload }) => ({
      ...state,
      history: [...state.history, payload],
    }))
    .addCase(actions.setChar, (state, { payload }) => ({
      ...state,
      typed: payload,
    }))
    .addCase(actions.setList, (state, { payload }) => ({
      ...state,
      ...shuffle(payload),
    }))
    .addCase(actions.appendInput, (state, { payload }) => ({
      ...state,
